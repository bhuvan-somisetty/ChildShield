...
const Login = () => {
  ...
  // Timeout fallback: if waking server takes > 30s, show retry option
  useEffect(() => {
    let timer;
    if (isWakingServer) {
  ...
  // ── PREMIUM LOADING UI while server is waking ──────────────────────────────
  if (isWakingServer) return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-primary)', gap: '24px' }}>
      <div style={{
      ...
      <div style={{ textAlign: 'center' }}>
        <p style={{ color: '#fff', fontWeight: '700', fontSize: '17px', marginBottom: '8px' }}>
          {wakeTimeout ? 'Server is taking longer than usual...' : 'Securing your session...'}
        </p>
        <p style={{ color: 'var(--text-muted)', fontSize: '13px' }}>
          {wakeTimeout ? 'Render free tier can take up to 90s to wake up.' : 'Waking AlphaGuard AI... please wait'}
        </p>
        {wakeTimeout && (
          <button
            onClick={() => { setIsWakingServer(false); setWakeTimeout(false); }}
            style={{ marginTop: '16px', padding: '10px 24px', borderRadius: '20px', background: 'var(--accent-primary)', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: '600' }}>
            Cancel & Retry
          </button>
        )}
      </div>
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-primary)', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '10%', left: '10%', width: '400px', height: '400px', background: 'var(--accent-purple)', filter: 'blur(150px)', opacity: 0.12, borderRadius: '50%', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: '10%', right: '10%', width: '400px', height: '400px', background: 'var(--accent-cyan)', filter: 'blur(150px)', opacity: 0.12, borderRadius: '50%', pointerEvents: 'none' }} />
      ...
      <div className="glass-panel animate-fade-in" style={{ width: '100%', maxWidth: '420px', padding: '40px', position: 'relative', zIndex: 1 }}>
        ...
        <form onSubmit={handleSubmit} autoComplete="off" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <button type="submit" disabled={submitting}
            style={{ background: 'var(--accent-primary)', color: '#fff', border: 'none', padding: '14px', borderRadius: '8px', fontWeight: '600', fontSize: '16px', marginTop: '8px', cursor: 'pointer', boxShadow: 'var(--shadow-neon-cyan)', transition: 'all 0.2s', opacity: submitting ? 0.7 : 1 }}>
        ...
        <p style={{ textAlign: 'center', fontSize: '14px', color: 'var(--text-muted)' }}>
          ...
          <Link to="/signup" style={{ color: 'var(--accent-primary)', textDecoration: 'none', fontWeight: '600' }}>Create one</Link>
        </p>
      </div>
    </div>
  );
